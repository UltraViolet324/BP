...
results2.append(['Nazev souboru', 'Velikost', 'Pixely', 'Bit/Pixel', 'PSNR'])
...
def main():
    ...
    compress_kakadu.tile = args.tile
...
class Compress_kakadu:
    ...
    def read_image(self, input_file):
        ...
        for self.nr_res in range(1, 11, 1):
            ...
            dist_file = path
            with open('time_' + self.folder + '-' + self.tile + '-' + str(self.nr_res) + '_c.txt', 'a') as f:
                run(["/usr/bin/time", "-f", "%U", "./kdu_compress", "-i", input_file, "-o", path, "Creversible=yes", "Stiles={" + self.tile + "}", "Clayers=" + str(self.nr_res)], stderr=f)
            quality_values.append(self.nr_res)
